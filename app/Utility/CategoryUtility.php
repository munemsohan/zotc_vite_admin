...
class CategoryUtility
{
    ...
    public static function get_immediate_children_minified($id, $with_trashed = false, $as_array = false)
    {
        $query = Category::without('category_translations')->select('id', 'name', 'icon')->where('parent_id', $id)->orderBy('order_level', 'desc');

        // If $with_trashed is true, include trashed (soft deleted) categories
        if ($with_trashed) {
        ...
        // Convert to an array if $as_array is true
        if ($as_array && !is_null($children)) {
