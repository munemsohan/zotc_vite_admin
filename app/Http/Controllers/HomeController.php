...
class HomeController extends Controller
{
    ...
    public function checkCustomDomain($customDomain)
    {
        ...
        $tlds = ['com', 'net', 'xyz', 'shop', 'store', 'site', 'app', 'online', 'info', 'co', 'top', 'live', 'io', 'biz', 'shopping', 'gifts', 'supply', 'zone', 'boutique', 'pro'];

        if (in_array($tld, $tlds)) {
            $url = "https://domaincheck.httpapi.com/api/domains/available.json?auth-userid=469705&api-key=5a0uwL3vmYboTrudDQbuUIbQDasdktAk&domain-name=" . $domain . "&tlds=" . $tld;
        } else {
            $url = "https://domaincheck.httpapi.com/api/domains/available.json?auth-userid=469705&api-key=5a0uwL3vmYboTrudDQbuUIbQDasdktAk&domain-name=" . $domain . "&tlds=com&tlds=net&tlds=xyz&tlds=shop&tlds=store&tlds=site&tlds=app&tlds=online&tlds=info&tlds=co&tlds=top&tlds=live&tlds=io&tlds=biz&tlds=shopping&tlds=gifts&tlds=supply&tlds=zone&tlds=boutique&tlds=pro";
        }
    ...
    public function buyDomainByGateway(Request $request)
    {
        $gateway = $request->gateway;
        $custom_domain = $request->domain;
        $epp_code = isset($request->epp_code) ? $request->epp_code : null;
        ...
        $success_url = route('domain.buy.success');
        $notify_url = 'https://zo.tc/notifywithpayment?purpose=domain_buy';
        $cancel_url = route('domain');
    ...
    public function domainBuySuccess()
    {
        flash(translate('Domain Buy and connected successfully !'))->success();

        return redirect()->route('admin.dashboard');
    }

    public function smsBuySuccess()
    {
        flash(translate('Sms Buy successful !'))->success();
        return redirect()->route('admin.dashboard');
    }
    public function balanceBuySuccess()
    {
        flash(translate('Balance Buy successful !'))->success();
        return redirect()->route('admin.dashboard');
    }

    public function planExtendSuccess()
    {
        flash(translate('Plan Extend complete successfully !'))->success();

        return redirect()->route('admin.dashboard');
    }

    public function lifetimePlanBuySuccess()
    {
        flash(translate('Lifetime Plan added to profile successfully !'))->success();
        return redirect()->route('admin.dashboard');
    }
    public function planBuySuccess()
    {
        flash(translate('Plan Upgrade complete successfully !'))->success();
    ...
    public function load_todays_deal_section()
    {
        ...
        if ((get_setting('show_todays_deal'))) {
    ...
    {
        $newest_products = Cache::remember(get_domain() . '_newest_products', 2592000, function () {
    ...
    public function trackOrder(Request $request)
    {
        if ($request->has('order_code')) {
            ...
            if ($order != null) {
                return view('frontend.track_order', compact('order', 'combined_order'));
            }
        }
    ...
    public function product(Request $request, $slug)
    {
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            ...
            if (get_setting('homepage_select') == 'modern') {
                return view('frontend.modern.product.product_details', compact('detailedProduct', 'product_queries', 'total_query', 'reviews', 'review_status'));
            } else {
    ...
    public function all_landing_pages()
    {
        $data['all_landing_pages'] = LandingPage::with('landingPageProducts.product')->orderBy('created_at', 'desc')->get();

        return view("frontend.landing.all_landing_page_list", $data);
    }
    ...
    public function checkBalanceKhudrobarta($api_key)
    {
        $response = Http::get('https://sms.one9.one/sms/api?action=check-balance&api_key=' . $api_key);
