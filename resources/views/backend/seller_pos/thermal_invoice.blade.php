	...
	<div>
		...
		<div style="padding: 1rem;padding-bottom: 0">
			...
			<div style="text-align: center;">
				<span style="font-size: 1.5rem;">{{ get_business_setting('website_name') }}</span><br>
				<span>{{ get_business_setting('contact_address') }}</span><br>
				<span>{{  translate('Email') }}: {{ get_business_setting('contact_email') }}</span><br>
				<span>{{  translate('Phone') }}: {{ get_business_setting('contact_phone') }}</span><br>
			</div>
		...
		<div style="padding: 0 1rem;">
			<div style="border-bottom:1px dotted #9c9ca1;padding-bottom: 0.25rem;">
				...
				<table>
					...
					<tr><td class="gry-color small">{{ translate('Address') }}: {{ $shipping_address->address ?? '' }}, {{ $shipping_address->city ?? '' }},
						@if(!empty($shipping_address->state)) {{ $shipping_address->state }} - @endif {{ $shipping_address->postal_code ?? '' }}, {{ $shipping_address->country ?? '' }}</td></tr>
