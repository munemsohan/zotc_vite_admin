    ...
    <div class="card">
        <form class="" id="sort_products" action="" method="GET">
            <div class="card-header row gutters-5">
                ...
                @endif
                @if ($type == 'All' && get_business_setting('vendor_system_activation') == 1)
                    <div class="col-md-2 ml-auto">
                        <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id"
                            name="user_id" onchange="sort_products()">
                            <option value="">{{ translate('All Sellers') }}</option>
                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            @if (get_business_setting('product_approve_by_admin') == 1 && $type == 'Seller')
                                    ...
                                    @else
                                        <div class="row justify-content-between">
                                            ...
                                            @if ($product->variant_product)
                                                <div class="">
                                                    @foreach ($product->stocks as $key => $stock)
                                                        ...
                                                        {{ $stock->variant }} -
                                                        <input type="number"
                                                            class="form-control p-0 h-20px text-center stock_input"
                                                            data-product="{{ $product->id }}"
                                                            data-stock="{{ $stock->id }}" value="{{ $stock->qty }}"
                                                            style="width: 50px" onchange="changeStock(this)">
                                                    @endforeach
                                                </div>
                                            @else
                                                @php
                                                ...
                                                <input type="number"
                                                    class="form-control p-0 h-20px text-center stock_input"
                                                    data-product="{{ $product->id }}"
                                                    data-stock="{{ optional($product->stocks->first())->id }}"
                                            ...
                                            @if ($qty <= $product->low_stock_quantity)
                                                <span
                                                    class="badge badge-inline badge-danger p-3">{{ translate('Low') }}</span>
                                            @endif
                                        </div>
                                    @endif
                                </td>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 'success') {
                        AIZ.plugins.notify('success', '{{ translate('Product stock updated successfully') }}');
                        location.reload();
                    } else {
