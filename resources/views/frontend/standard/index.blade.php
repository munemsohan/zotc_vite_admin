@extends('frontend.standard.layouts.app')
...
@section('content')
    ...
    <!-- Sliders -->
    @if (get_setting('show_categories_sliders'))
        <div class="home-banner-area mb-3" style="">
            <div class="container">
                <div class="d-flex flex-wrap position-relative">
                    @if (!get_setting('show_full_sliders'))
                    ...
                    <!-- Sliders -->
                    <div class="home-slider {{ get_setting('show_full_sliders') ? 'slider-full' : '' }} ">
                                ...
                                @foreach ($sliders as $key => $slider)
                                    <div class="carousel-box">
                                        <a
                                            ...
                                            <!-- Image -->
                                            <img class="d-block mw-100 img-fit overflow-hidden h-180px h-md-320px h-lg-320px overflow-hidden"
                                                src="{{ get_setting('show_full_sliders') ? my_asset($slider->file_name) : my_asset($slider->thumbnail) }}"
    ...
    @if (get_setting('show_scrolling_text'))
        <section class="my-0">
            <div class="container">
                <!-- Four Elements -->
                <marquee class="bg-primary h-35 text-white pt-2 rounded">
                    <h6 class="fw-700">{{ base64_decode(get_setting('scrolling_text')) }}</h6>
                </marquee>
            </div>
        </section>
    @endif

    @if (get_setting('show_4element'))
        @php
        ...
        <section class="mb-2 mb-md-3 mt-2 mt-md-3">
            <div class="container">
                <!-- Four Elements -->
                <div class="row mb-2">
                    <div class="text-center col-6 col-md-3 p-md-0">
                        <i class="{{ $four_elements[0] }} fs-30 mb-3" style="color: {{ $four_elements[1] }}"></i>
                        <h5>{{ $four_elements[2] }}</h5>
                        <p>{{ $four_elements[3] }}</p>
                    </div>

                    <div class="text-center col-6 col-md-3 p-md-0">
                        <i class="{{ $four_elements[4] }} fs-30 mb-3" style="color: {{ $four_elements[5] }}"></i>
                        <h5>{{ $four_elements[6] }}</h5>
                        <p>{{ $four_elements[7] }}</p>
                    </div>

                    <div class="text-center col-6 col-md-3 p-md-0">
                        <i class="{{ $four_elements[8] }} fs-30 mb-3" style="color: {{ $four_elements[9] }}"></i>
                        <h5>{{ $four_elements[10] }}</h5>
                        <p>{{ $four_elements[11] }}</p>
                    </div>

                    <div class="text-center col-6 col-md-3 p-md-0">
                        <i class="{{ $four_elements[12] }} fs-30 mb-3" style="color: {{ $four_elements[13] }}"></i>
                        <h5>{{ $four_elements[14] }}</h5>
                        <p>{{ $four_elements[15] }}</p>
                    </div>
                </div>
            </div>
        </section>
    ...
    @if (get_setting('show_featured_categories'))
        <!-- Featured Categories -->
        @if (count($featured_categories) > 0 && get_setting('show_featured_categories'))
            <section class="mb-2 mb-md-3 mt-2 mt-md-3">
                <div class="container">
                    <div class="bg-white">
                        <!-- Top Section -->
                    ...
                    <div class="bg-white px-3">
                        <div class="row border-top border-right">
                            @foreach ($featured_categories->take(6) as $key => $category)
                                ...
                                @endphp
                                <div class="col-6 col-md-3 border-left border-bottom py-3 py-md-2rem">
                                    <div class="d-sm-flex text-center text-sm-left">
                                        <div class="mb-3">
                                            <a href="{{ route('products.category', $category->slug) }}">
                                                <img src="{{ isset($category->bannerImage->file_name) ? my_asset($category->bannerImage->file_name) : static_asset('assets/img/placeholderx200.webp') }}"
                                                    class="lazyload w-150px h-auto mx-auto has-transition" alt=""
                                                    onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholderx200.webp') }}';">
                                            </a>
                                        </div>
    ...
    @if (get_setting('show_featured_products'))
        <!-- Featured Products -->
        <div id="section_featured"></div>
    @endif

    @if (get_setting('show_home_banner2_images'))
    ...
    @if (get_setting('show_home_banner3_images'))
    ...
    <!-- Cupon -->
    @if (get_setting('coupon_system') == 1 && get_setting('show_coupon'))
        <div class="mb-2 mb-md-3 mt-2 mt-md-3"
                                            ...
                                            <div class="col-4 col-md-5 mb-3 mb-md-0">
                                                <a href="{{ route('customer.product', $classified_product->slug) }}"
                                                    ...
                                                    <img class="img-fluid lazyload mx-auto has-transition"
                                                        ...
                                                        data-src="{{ isset($classified_product->thumbnail->file_name) ? my_asset($classified_product->thumbnail->file_name) : static_asset('assets/img/placeholderx200.webp') }}"
                        ...
                        @foreach ($brands as $brand)
                            <div
                                ...
                                <a href="{{ route('products.brand', $brand->slug) }}" class="d-block p-sm-3">
                                    <img src="{{ isset($brand->brandLogo->file_name) ? my_asset($brand->brandLogo->file_name) : static_asset('assets/img/placeholderx200.webp') }}"
