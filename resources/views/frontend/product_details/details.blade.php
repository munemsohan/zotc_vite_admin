<div class="text-left">
    ...
    <div class="d-flex flex-wrap align-items-center my-2">
        <span class="text-secondary fs-14 fw-400 mr-4 w-50px">{{ translate('Category') }}</span><br>
        <a href="{{ route('products.category', $detailedProduct->main_category->slug) }}"
            class="text-reset hov-text-primary fs-14 fw-700">{{ $detailedProduct->main_category->name }}</a>
    </div>
                                ...
                                @foreach (json_decode($detailedProduct->colors) as $key => $color)
                                    <label class="aiz-megabox pl-0 mr-2 mb-0" data-toggle="tooltip"
                                        ...
                                        <input type="radio" name="color"
                                            ...
                                            @if ($key == 0) checked @endif>
                                        <span
                                            class="aiz-megabox-elem rounded-0 d-flex align-items-center justify-content-center p-1 h-auto">
                ...
                @else
                    @if ($qty > 0)
                        ...
                        <button type="button"
                            ...
                            onclick="showProductAddToCartModal({{ $detailedProduct->id }}, {{ $detailedProduct->variant_product }})">
                            <i class="la la-shopping-cart"></i> {{ translate('Buy Now') }}
                        </button>
                    @endif
                @endif
                <button type="button" class="btn btn-secondary out-of-stock fw-600 {{ $qty > 0 ? 'd-none' : '' }}"
                    disabled>
