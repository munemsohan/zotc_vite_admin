class OpenVerse {
    ...
    constructor() {
        ...
        this.key = {
            client_secret:
                "LSXRdNkOaga5QF8T07xgxqDZq8vijvmLCpNZnSZXSTbmAPl13B5iCpGJlrlXiKbOLXI8k2xf8C8BUOuPMre09XwaTi2tFJwVEBgAVfkMsg4YD76cENxSp9uzOkWIGntd",
            client_id: "U03QwdlXwhHKlZcFakAI2dsthpV7bNslEUtH2fEz",
            name: "munem3.zo.tc",
    ...
    authenticate() {
        let url = $("#openverse_authenticate_url").val(); // Correct for Blade templates
        let self = this;
        ...
        $.ajax({
            ...
            self.accessToken = data.response.access_token;
...
class OpenVerseDisplay extends OpenVerse {
    ...
    page(pageNo) {
        this.pageNo = pageNo;
        this.filtersParameters =
            $("#openverse-form").serialize() + "&page=" + pageNo;
        this.showLoading();
    ...
    init() {
        ...
        $("#MediaModal .modal-footer .align-left").append(
            self.paginationContainer()
        );
        $("#openverse-filters").prepend(self.getFiltersHtml());
        $("#openverse-search-btn").click(function (e) {
            self.search();
            e.preventDefault();
        });
